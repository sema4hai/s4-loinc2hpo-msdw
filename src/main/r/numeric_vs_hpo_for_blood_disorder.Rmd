...
## Goal
Compare the performance of raw lab values vs HPO interpretations for seme blood disorders
...
df_cohort <- get_df_from_db_or_cache(cache_dir = CACHE_DIR, file_name = 'aplastic_anemia_diagnoses.csv', db_access_callback = dbGetQuery, conn = dsca, statement = sql, force_rerun = FALSE)
...
df_labs <- get_df_from_db_or_cache(cache_dir = CACHE_DIR, file_name = "labs_aplastic_anemia.csv", db_access_callback = dbGetQuery, conn=dsca, statement = sql, force_rerun = FALSE)
df_labs %>% nrow()
```
...
ggsave(filename = 'cache/anemia_labs_raw_data_distribution_by_rel_date.png', plot = p, width = 5, height = 5)
```

![liver function labs](./cache/anemia_labs_raw_data_distribution_by_rel_date.png)
