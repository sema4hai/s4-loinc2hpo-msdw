...
ggsave(filename = glue('{CACHE_DIR}/CRC_labs_raw_data_distribution_by_rel_date.png'), plot = p, width = 8, height = 6)
```
...
ggsave(glue("{CACHE_DIR}/plot_CRC_before_matching.png"), width = 2, height = 1.5)
p
```
```{r}
require(MatchIt)
...
run_matchit <- function(filepath, force_rerun) {
  if (force_rerun | !file.exists(filepath)){
    ...
    # save the matchit object in cache
...
ggsave(glue("{CACHE_DIR}/plot_CRC_after_matching.png"), plot = p, width = 2, height = 1.5)
...
ggsave(filename = glue('{CACHE_DIR}/plot_CRC_age_gender_race_after_matching.png'), plot = plot_match, width = 6, height = 5)
plot(plot_match)
```
...
crc_case_matched_controls_analysis %>% write.csv(file = glue('{CACHE_DIR}/crc_case_matched_controls_analysis.csv'), row.names = FALSE)
