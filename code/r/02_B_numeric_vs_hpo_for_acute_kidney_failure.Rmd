...
## Goal
Compare the performance of raw lab values vs HPO interpretations for acute kidney failure.

The analysis process is very similar to the liver function analysis.

Algorithm:
- Define a cohort of patients coded with acute kidney failure as the primay diagnosis of a visit;
...
```{r}
sql <- read_file('../sql/acute_kidney_failure_cohort.sql')
print(sql)
...
df_cohort <- get_df_from_db_or_cache(cache_dir = CACHE_DIR, file_name = 'acute_kidney_failure_diagnoses.csv', db_access_callback = dbGetQuery, conn = dsca, statement = sql, force_rerun = FORCE_RERUN)
...
```{r}
sql <- read_file('../sql/get_labs_for_acute_kidney_failure.sql')
df_labs <- get_df_from_db_or_cache(cache_dir = CACHE_DIR, file_name = "labs_acute_kidney_failure.csv", db_access_callback = dbGetQuery, conn=dsca, statement = sql, force_rerun = FORCE_RERUN)
df_labs %>% nrow()
```
...
p1 <- plot_lab_raw_values_vs_rel_date(lab_loinc = '2160-0', ylim = c(0, 20)) + ggtitle('Serum creatinine')

p2 <- plot_lab_raw_values_vs_rel_date(lab_loinc = '3094-0', ylim = c(0, 150)) + ggtitle('Urea nitrogen in serum')

p3 <- plot_lab_raw_values_vs_rel_date(lab_loinc = '6299-2', ylim = c(0, 200)) + ggtitle('Urea nitrogen in blood')

p4 <- plot_lab_raw_values_vs_rel_date(lab_loinc = '2823-3', ylim = c(0, 10)) + ggtitle('Potassium in serum')

# seems the test strip only reports positive or negative outcomes
p5 <- plot_lab_raw_values_vs_rel_date(lab_loinc = '5804-0', ylim = c(0, 500)) + ggtitle('protein in urine')

# very sparse data
p6 <- plot_lab_raw_values_vs_rel_date(lab_loinc = '2889-4', ylim = c(0, 200)) + ggtitle('Protein in 24h urine')

# very very sparse data
p7 <- plot_lab_raw_values_vs_rel_date(lab_loinc = '2888-6', ylim = c(0, 500)) + ggtitle('Protein in urine')

p <- gridExtra::arrangeGrob(p1, p2, p3, p4, p5, p6, p7, ncol = 3)

ggsave2(filenames = c(glue('{OUT_DIR}/kidney_labs_raw_data_distribution_by_rel_date.png'), glue('{OUT_DIR}/kidney_labs_raw_data_distribution_by_rel_date.pdf')), plot = p, width = 6, height = 6)
```

![liver function labs](./../../out/kidney_labs_raw_data_distribution_by_rel_date.png)
...
df_labs_and_outcomes <- df_labs %>% filter(loinc %in% c('2160-0', '3094-0', '6299-2', '2823-3')) %>%
  as_tibble() %>%
...
kidney_analysis <- df_labs_and_outcomes %>%
  group_by(loinc) %>%
  nest() %>%
  ungroup() %>% 
...
write.csv(kidney_analysis, file = glue('{OUT_DIR}/acute_kidney_failure_analysis.csv'), row.names = FALSE)

kidney_analysis
```

```{r}
df_one_loinc <- df_labs_and_outcomes %>% filter(loinc == '2160-0')
...
roc_comparison$H <- roc_comparison
